...
: step ( fn -- )                   \ alter intensity in response to turn
  intensity @ stepinc rot execute  \ apply function with increment
...
: timerA0-irq-handler \ rotary encoder debounce code
  ...
  \ Have we debounced?
  \ Button
  buttonstate @ debounce_check = if sleepreset then
  \ Rotary Encoder
